...
customtest ('Client App page _End to End', async ({page, testDataForOrder}) => {
  ...
  await loginPage.validLogin (
    testDataForOrder.username_input,
    testDataForOrder.password_input
  );
  ...
  if (paymentMode.includes ('Credit Card')) {
    ...
    await creditCardNumber.fill (testDataForOrder.CcNumber);
    ...
    await expiry_Month_input.selectOption ({label: testDataForOrder.expiry_Month});

    await expect (expiry_Date_input).toBeVisible ();
    await expiry_Date_input.selectOption ({label: testDataForOrder.expiry_Date});
    //page.pause();

    await expect (CvvCode_input).toBeVisible ();
    await CvvCode_input.fill (testDataForOrder.CvvCode);
